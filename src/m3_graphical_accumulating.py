...
def draw_parallel_lines(n, point, length, window):
    ...
    window_1 = window
    for i in range(n):
        line_x = rg.Line(rg.Point(point_x, point_y), rg.Point((point_x+length), point_y))
        line_x.attach_to(window_1)
        window.render()
        point_y += 30
...
def draw_lines(n, point, window):
    ...
    window_1 = window
    for i in range(n):
        line_x = rg.Line(rg.Point(point_x, point_y), rg.Point((point_x + 100), point_y))
        line_x.attach_to(window_1)
        window.render()
        point_y += (100/n)
